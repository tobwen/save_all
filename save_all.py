                    ...
                    for layer in project.mapLayers().values():
                        ...
                        if layer.type() == QgsMapLayerType.VectorLayer:
                            ...
                            # Save SHP layers as SHP files
                                ...
                                if not os.path.exists(output_file):
                                    error, error_string = QgsVectorFileWriter.writeAsVectorFormat(layer, output_file, "utf-8", layer.crs(), "ESRI Shapefile", onlySelected=False, symbologyExport=Qgis.FeatureSymbologyExport.NoSymbology)
                                    if error != QgsVectorFileWriter.NoError:
                                        # Failure message
                                        iface.messageBar().pushMessage("Failed: ", "Layer '{}' was not saved. Error: {}".format(layer.name(), error_string), level=2)
                                        failedSaves.append(layer_name)
